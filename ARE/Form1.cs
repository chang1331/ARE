    ...
    public partial class frmSalones : Form
    {
       ...
       // Dictionary<int,int> resHorario= new Dictionary<int, int>() ;
        ...
        private void btnCalcula_Click(object sender, EventArgs e)
        {
            ...
            {
                String linea = txtResHor.Lines[j];
                
                if (linea[1] == '<')
                {
                    c = Convert.ToChar(linea[0]);
                    index = char.ToUpper(c) - 65;

                    Console.WriteLine(index);
                    for (int i = (int)Char.GetNumericValue(linea[3])-2 ; i > 0; i--)
                    {
                ...
                {
                    index = (int)Convert.ToChar(linea[0])-65;
                    Console.WriteLine(index);
                    for (int i=0; i < (int)Char.GetNumericValue(linea[3])-1; i++) {
                        resHorario[index].Add(i);
                    }
                }
            }
            ...
            for (int i = 0; i < horario.Count; i++)
            {
                ...
                {
                    Console.Write(string.Format("{0} ", horario[i][j]));
                }
                Console.Write(Environment.NewLine);
            }
        ...
        public void areBi()
        {
            for (int k=0;k<n;k++) {
                ...
                Console.WriteLine("indice de salon con mas conex " + iMax);
                int x = 0;
                ...
                horario[x].Add(iMax);
                Console.WriteLine("agregué " + iMax + " a horario " + x);
